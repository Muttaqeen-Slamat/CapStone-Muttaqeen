  ...
  actions: {
    async register(context, payload) {
      try {
        const { msg, token } = await (
          await axios.post(`${capUrl}users/register`, payload)
        ).data;
        if (token) {
    ...
    async login(context, payload) {
      try {
        const response = await axios.post(
          "https://capstone-muttaqeen.onrender.com/users/login",
          payload
        );
    ...
    async logout({ commit }) {
      try {
        ...
        document.cookie =
          "userAuthenticated=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
    ...
    async fetchProducts(context) {
      try {
        let { results } = (await axios.get(`${capUrl}products`)).data;
        if (results) {
    ...
    async fetchProduct(context, payload) {
      try {
        let { result } = (await axios.get(`${capUrl}products/${payload.id}`))
          .data;
        if (result) {
    ...
    async fetchCart(context, payload) {
      try {
        let { result } = (await axios.get(`${capUrl}cart/${payload.id}`)).data;
        if (result) {
          context.commit("setCart", result);
        } else {
          // Handle if cart is not found
        }
      } catch (e) {
        // Handle error
      }
    ...
    async fetchCarts(context) {
      try {
        let { results } = (await axios.get(`${capUrl}cart`)).data;
        if (results) {
    ...
    async addCart(context, payload) {
      try {
        let { msg } = await axios.post(`${capUrl}cart/add`, payload);
        if (msg) {
    ...
    async updateCart(context, { id, data }) {
      try {
        let { update } = await axios.patch(`${capUrl}cart/update/${id}`, data);
        if (update) {
    ...
    async deleteCart({ commit, dispatch }, payload) {
      try {
        await axios.delete(`${capUrl}cart/delete/${payload.id}`);
        commit("setCarts");
        dispatch("fetchCarts");
        Swal.fire({
          title: "Update Successful",
